...
Description: Chapter 6:  RL example for lane keeping problem in a circle road
             Debug the trained agent with render
"""
# =================== load package ====================
import argparse
import os
...
for step in range(400):  # s: 𝜌, 𝜓, 𝑢, 𝑣, 𝜔
    steer = 0.09 + 0.05 * s[0] + 0.01 * s_int[0] - 1.0 * s[1] - 0.1 * s_int[1] - 0.1*(s[4]-0.15)
    acc = -0.5 * s[2] - 0.05 * s_int[2]
    a = np.array([[steer, acc]])
