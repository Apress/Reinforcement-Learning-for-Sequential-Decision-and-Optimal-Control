...
Description: Chapter 6:  RL example for lane keeping problem in a circle road
             Actor-Critic algorithm with deterministic policy and action value function
...
from lib.agent import DeterminePolicy, ActionValue
...
# ==================== Set log path ====================
log_dir = "./Results_dir/" + "det_act/" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
print(log_dir)
...
for iter_index in range(arg.iter_num):
    ...
    for path in paths:
        reward_ep = path["reward"]
        reward_ep = reward_ep.reshape(-1, 1)

    # combined all the episodes together
    ...
    pi = actor.forward(state_tensor)
    q_pi = critic(state_tensor, pi)
    a_loss = -q_pi.mean()
    opt_actor.zero_grad()
    a_loss.backward()
    opt_actor.step()
    ...
    # Critic loss & update
    for _ in range(20):
        # compute r + gamma * Q(s', a')
        action_tensor = torch.Tensor(action_batch).reshape(-1, A_DIM)
        q = critic(state_tensor, action_tensor)
    ...
    writer.add_scalar("tag/c_loss", c_loss.item(), iter_index)
    ...
    if iter_index % 5 == 0:
        tar, start = policy_evaluate(env, actor, "det")
        tars.append(tar)
